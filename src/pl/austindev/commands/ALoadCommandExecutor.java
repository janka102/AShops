...
public class ALoadCommandExecutor extends ASCommandExecutor {

	public ALoadCommandExecutor(AShops plugin) {
		super(plugin, ASCommand.ALOAD);
	}
	...
	protected void run(CommandSender sender, Command command, String label,
		...
		try {
			getShopsManager().load();
			tell(sender, ASMessage.FILE_LOADED);
