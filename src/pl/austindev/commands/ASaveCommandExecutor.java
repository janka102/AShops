...
public class ASaveCommandExecutor extends ASCommandExecutor {
	public ASaveCommandExecutor(AShops plugin) {
		super(plugin, ASCommand.ASAVE);
	}
	...
	protected void run(CommandSender sender, Command command, String label,
		...
		try {
			getShopsManager().save();
			tell(sender, ASMessage.FILE_SAVED);
